...
namespace OnSale.Web.Data
{
    public class SeedDb
    {
        ...
        {
            await CheckUserAsync("1001", "fabiousuga@gmail.com", UserType.Admin);
        }
            ...
            UserType userType)
        {
            ...
            if (user == null)
            {
                user = new User
                {
                    FirstName = "Nelson", //randomUser.Name.First,
                    LastName = "Usuga",//randomUser.Name.Last,
                    Email = email,
                    UserName = email,
                    PhoneNumber ="3186529777", //randomUser.Cell,
                    Address ="Diagona 59 #10-122", //$"{randomUser.Location.Street.Number}, {randomUser.Location.Street.Name}",
                    Document = document,
                    UserType = userType,
                    City = await _context.Cities.FindAsync(cityId),
                    ImageId =new Guid(), //imageId,
                    Latitude =2020, //double.Parse(randomUser.Location.Coordinates.Latitude),
                    Logitude =3030 //double.Parse(randomUser.Location.Coordinates.Longitude)
                };
        ...
        private async Task AddProductAsync(Category category, string description, string name, decimal price, string[] images, User user)
        {
            ...
            foreach (string image in images)
            {
                ...
                Guid imageId = new Guid(); //await _blobHelper.UploadBlobAsync(path, "products");
        ...
        private async Task AddCategoryAsync(string name)
        {
            ...
            Guid imageId = new Guid(); //await _blobHelper.UploadBlobAsync(path, "categories");
